...
elements.startEl.addEventListener("click", (evt) => {
  startGame();
  evt.target.blur();
});
elements.manualEl.addEventListener("click", () => {
  if (!state.gameStart) {
    elements.manualContentEl.show();
  }
  evt.target.blur();
});
elements.closeEl.addEventListener("click", () => {
  elements.manualContentEl.close();
  evt.target.blur();
});
...
function finishGame(beginIdx) {
  ...
  setTimeout(function () {
    currentImgEl.classList.add("hide");
  }, 0);
...
function renderPicUpdate() {
  for (let i = state.beginIdx - 1; i <= state.endIdx; i++) {
    ...
    elements.picsEl[
      i
    ].firstChild.style.transform = `rotate(${currentRotationUnit}deg)`;
    currentImgEl.style.borderColor = state.pictures[i].getColor();
